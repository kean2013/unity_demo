        ...
        virtual public IHtmlObject CreateObject(RichTextField owner, HtmlElement element)
        {
            ...
            else if (element.type == HtmlElementType.Select)
            {
                if (_selectPool.Count > 0 && _poolManager != null)
                {
                    ...
                    ret = new HtmlSelect();
            }else if (element.type == HtmlElementType.Audio)
            {
                ret = new HtmlEvent("audio");
            }else if (element.type == HtmlElementType.Avatar)
            {
                ret = new HtmlEvent("avatar");
            }else if (element.type == HtmlElementType.Event)
            {
                ret = new HtmlEvent("event");
            }
