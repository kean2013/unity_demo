...
namespace FairyGUI
{
    ...
    public class UIPackage
    {
        ...
        static LoadResource _loadFromStreamPath = (string name, string extension, System.Type type, out DestroyMethod destroyMethod) =>
        {
            destroyMethod = DestroyMethod.Unload;
            return Resources.Load(name, type);//后面改成addressable TODO:wangrensi
        };

        static LoadResource _loadFromPersistentDataPath = (string name, string extension, System.Type type, out DestroyMethod destroyMethod) =>
        {
            destroyMethod = DestroyMethod.Unload;
            return Resources.Load(name, type);//后面改成addressable TODO:wangrensi
        };
        ...
        public static UIPackage AddPackage(string descFilePath)
        {
            if (descFilePath.StartsWith("Assets/"))
            {
                ...
                var p = AddPackage(descFilePath, _loadFromPersistentDataPath);
                if(p != null)
                {
                    return p;
                }
                return AddPackage(descFilePath, _loadFromStreamPath);
#endif
            }
        ...
        /// <returns>UIPackage</returns>
        public static UIPackage AddLuaPackage(string fileName)
        {
#if UNITY_EDITOR
            return AddPackage($"Assets/Resources/UIPackage/{fileName}");
#else

            return AddPackage($"UIPackage/{fileName}", _loadFromResourcesPath);
#endif
        }
        ...
        public static GObject CreateObject(string pkgName, string resName)
        {
            ...
            if (pkg != null)
            {
                var go = pkg.CreateObject(resName);
                go.gameObjectName = pkgName;
                return go;
            }
            else
                return null;
